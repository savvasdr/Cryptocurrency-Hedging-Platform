...
import { Button, Input, Item, Grid, Label, Icon} from 'semantic-ui-react'
...
class App extends Component {
  ...
  async componentDidMount() {
	...
	if (haveRecentPrice) {
		this.setState({ haveLatestEthPrice: "OKKK! You have a recent Ether/USD price. You can continue..." });
	} else {
		this.setState({ haveLatestEthPrice: "Ohhh you don't have a recent Ether/USD price. Plese click on 'GET LATEST PRICE' button!!!" });
	}
	...
	const storeOfValueOpenOrders = await Logic.methods.getUserStoreOfValueOpenOrders().call({from: accounts[0]}).then();
	...
	this.setState({ userStoreOfValueOpenOrdersID: storeOfValueOpenOrdersArray });
	
	// Get all 'Increase price bet' open orders ID's of the user
	const increasePriceOpenOrders = await Logic.methods.getUserPriceIncreaseOpenOrders().call({from: accounts[0]}).then();
	...
	this.setState({ userIncreasePriceOpenOrdersID: increasePriceOpenOrdersArray });
	...
	for (var key in storeOfValueOpenOrdersArray) { 
		storeOfValueAmountArray.push((await Data.methods.getStoreOfValueAmountByID(storeOfValueOpenOrdersArray[key]).call({from: accounts[0]}))/10**18);
	}
	this.setState({ userStoreOfValueOpenOrdersAmount: storeOfValueAmountArray });
	...
	for (var key in increasePriceOpenOrdersArray) { 
		increasePriceAmountArray.push((await Data.methods.getIncreasePriceAmountByID(increasePriceOpenOrdersArray[key]).call({from: accounts[0]}))/10**18);
	}
	this.setState({ userIncreasePriceOpenOrdersAmount: increasePriceAmountArray });

	// Get all 'Matched orders' of the user
	const matchedOrders = await Logic.methods.getUserMatchedOrders().call({from: accounts[0]}).then();
	...
	this.setState({ userMatchedOrdersID: matchedOrdersArray });
	...
	for (var key in matchedOrdersArray) { 
		matchedAmountObject.push((await Data.methods.getMatchedOrderInformationtByID(matchedOrdersArray[key]).call({from: accounts[0]})));
	}
	...
	this.setState({ userMatchedOrdersAmount});
	this.setState({ userMatchedOrdersType});
	this.setState({ userMatchedOrdersEtherPrice});
 }  
  ...
  updatePrice = async event => {
	...
	this.setState({ haveLatestEthPrice: <div dangerouslySetInnerHTML={{ __html: '<div class="ui segment"><div class="ui active transition visible dimmer"><div class="content"><div class="ui loader"></div></div></div><img src="https://react.semantic-ui.com/images/wireframe/short-paragraph.png" class="ui image"/></div>' }} /> });
  ...
  addNewStoreOfValueOrder = async event => {
	...
	this.setState({ waitForNewStoreOfValueOrder: <div dangerouslySetInnerHTML={{ __html: '<div class="ui segment"><div class="ui active transition visible dimmer"><div class="content"><div class="ui loader"></div></div></div><img src="https://react.semantic-ui.com/images/wireframe/short-paragraph.png" class="ui image"/></div>' }} /> });
